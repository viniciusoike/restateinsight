...
description: "Escalas, legendas e cores são elementos essenciais numa boa visualização. Este post apresenta a lógica das funções que controlam as escalas do gráfico e as suas cores com diversos exemplos. Também se apresenta a "
...
O exemplo abaixo mostra a relação entre renda e escolaridade, olhando especificamente para a taxa de indivíduos com ensino superior. A cor de cada ponto ilustra a densidade populacional de cada Zona em que os pontos mais escuros são as Zonas mais densas. Note o uso da função `scale_y_log10`.
...
A função `scale_*_binned` permite uma visualização um pouco mais simplificada ao agrupar os dados em grupos.
...
É possível usar escalas customizadas, de outras funções, usando uma função anônima.
