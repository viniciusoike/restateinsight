...
Suponha, por exemplo, que se queira desenhar uma linha reta no eixo-x. Para isto precisamos estruturar um `data.frame` e informar as coordenadas. Note como no exemplo abaixo, não preciso repetir o valor `y = 0` já que o `tibble` "recicla" este número para que ele tenha o mesmo comprimento (`length)` que o vetor `x`.
...
As funções `round`, `paste`, `paste0` e `format` podem ser muito úteis para melhor apresentar resultados numéricos[^6]. No exemplo abaixo eu ajusto o dado da coluna `share`: eu multiplico o valor por 100, arredondo até a primeira casa decimal, uso a vírgula como separador de decimal e coloco um sinal de "%" após o número.
...
Por fim, monto mais um exemplo de como combinar gráficos de coluna com texto. No gráfico abaixo eu mostro a eficiência (milhas por galão) de um subconjunto de carros (os que tem 4 cilindradas). Além disso, eu ordeno o nome dos carros pela variável `mpg` e depois inverto esta ordem; esta inversão é importante porque eu viro o gráfico de lado com a função `coord_flip`.
