...
description: "O tidyverse é um conjunto de pacotes focados em limpeza e extração de dados, que se tornou extremamente popular. O sucesso do tidyverse se deve à sua consistência em torno de princípios básicos. Neste post, discuto o estado atual do tidyverse e o que me parece ser sua filosofia."
image: "/static/images/thumbnails/tidyverse_philosophy.png"
image-alt: "/static/images/thumbnails/tidyverse_philosophy.png"
execute:
  eval: false
  message: false
  warning: false
...
tab_packs <- tab |>
  ...
  mutate(
    package_type = factor(
      package_type,
      levels = c("non_tidy", "depends", "imports")
    )
  )
...
p2 <- ggplot(tab_packs, aes(x = year, y = count, fill = package_type)) +
  geom_area() +
  geom_hline(yintercept = 0) +
  scale_x_continuous(breaks = 2008:2023, expand = expansion(0)) +
  scale_y_continuous(labels = scales::label_number(big.mark = ".")) +
  scale_fill_manual(
    name = "",
    values = c("#ffb703", "#023047", "#126782"),
    labels = c("Não utiliza", "Depends", "Imports")
  ) +
  labs(
    title = "Crescimento dos pacotes que usam tidyverse",
    x = NULL,
    y = "Pacotes Publicados",
    subtitle = "Total de pacotes publicados no CRAN que dependem de algum pacote (core) do tidyverse por ano de publicação.",
