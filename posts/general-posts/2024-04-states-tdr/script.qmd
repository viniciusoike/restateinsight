...
plot_map <- map_states + plot_annotation(
  ...
  subtitle = stringr::str_wrap("Razão de dependência jovem e idosa no Brasil por estado (2022). Sul e Sudeste são os mais envelhecidos enquanto Norte ainda é jovem. Nordeste e Centro-Oeste são mais heterogêneos, mas já apresentam sinais de envelhecimento populacional.", 80)
) &
  theme(
    ...
    plot.subtitle = element_text(size = 10, hjust = 0, family = "Lato", color = "gray20")
    )
```
...
sub <- sub |> 
  ...
  summarise(
    total = sum(count), .by = c("code_state", "sex", "age_group_lump")
  ) %>%
...
plot_pyramid <- ggplot(sub, aes(x = age_group_lump, y = share, fill = sex, color = sex)) +
  ...
  labs(
    ...
    subtitle = stringr::str_wrap("Amazonas tem população jovem, RDI baixo e RDJ elevado.
    ...
    Santa Catarina compensa população idosa com maior população adulta.", 91),
    x = NULL,
