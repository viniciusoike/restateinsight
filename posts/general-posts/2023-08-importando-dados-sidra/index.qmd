...
Vamos importar a população de todos os municipios em 2021. Note que no código abaixo o argumento do ano é inserido como um `character`. A tabela já vem formatada dentro do R.
...
Ainda que os dados não estejam "sujos", eles também não estão num formato agradável para se trabalhar. Vamos fazer uma limpeza básica dos dados. Primeiro, vamos simplificar os nomes das colunas e selecionar apenas as colunas de maior interesse.
...
Um detalhe curioso é que a função não retorna erros quando o período extrapola os valores disponíveis nos dados. Assim a função abaixo retorna apenas os valores de 2021 (ou até o ano mais recente disponível).
...
# Pre-compute Jenks breaks (7 classes = 8 break points)
breaks <- BAMMtools::getJenksBreaks(
  na.omit(resp_bairros$age_25_39),
  k = 8
)
breaks <- round(breaks, 0)

# Inferno color palette binned to Jenks breaks
pal <- colorBin(
  palette = "Spectral",
  domain = resp_bairros$age_25_39,
  bins = breaks,
  na.color = "transparent"
)

# HTML popup content with all age groups
popup_content <- sprintf(
  "<b>%s</b><br>
   Menos de 25 anos: %.1f%%<br>
   25 a 39 anos: %.1f%%<br>
   40 a 59 anos: %.1f%%<br>
   60 a 79 anos: %.1f%%<br>
   80 anos ou mais: %.1f%%",
  resp_bairros$name_neighborhood,
  resp_bairros$age_25,
  resp_bairros$age_25_39,
  resp_bairros$age_40_59,
  resp_bairros$age_60_79,
  resp_bairros$age_80
)
popup_content <- lapply(popup_content, htmltools::HTML)

leaflet(resp_bairros) |>
  addProviderTiles("CartoDB.Positron") |>
  setView(lng = -51.179152, lat = -30.025976, zoom = 13) |>
  addPolygons(
    fillColor    = ~pal(age_25_39),
    fillOpacity  = 0.8,
    color        = "white",
    weight       = 2,
    opacity      = 1,
  ...
  addLegend(
    ...
    pal       = pal,
    values    = ~age_25_39,
    title     = "% domicílios chefiados<br>por pessoas de 25-39 anos",
