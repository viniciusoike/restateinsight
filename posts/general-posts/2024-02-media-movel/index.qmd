...
No exemplo abaixo vamos importar 9 séries do Índice de Produção Industrial (IPI) e aplicar uma média móvel 12x2 em cada uma delas. O código abaixo mostra como importar as séries e empilhá-las num único `tibble`. Para manipular os dados vamos utilizar o popular pacote `dplyr`[^3].
...
ggplot(ttrc, aes(x = date, y = value, color = series)) +
  ...
  geom_line(data = dplyr::filter(ttrc, series != "close"), lwd = 0.9) +
  scale_color_manual(
    name = "",
    values = c("#073B4C", "#FFD166", "#0CB0A9", "#F4592A"),
