...
execute:
  warning: false
  message: false
image: "/static/images/posts/2025-07-modelo-arima/plots-series-tempo.png"
image-alt: "/static/images/posts/2025-07-modelo-arima/plots-series-tempo.png"
---
...
Neste post, tentei manter o código o mais simples possível usando somente o pacote `forecast`[^1], que é excelente para o tratamento de séries de tempo univariadas[^2]. Infelizmente, as funções para o diagnóstico de resíduos estão espalhadas em diversos pacotes. Abaixo segue a lista dos pacotes necessários para acompanhar este post[^3].
...
library(forecast)
library(urca)
other_packages <- c("tseries", "lmtest", "FinTS", "ggplot2")
```
...
theme_ts = theme_minimal(base_family = font, base_size = 11) +
  theme(
...
# opcional: para aplicar o tema em todos os gráficos
set_theme(theme_ts)
