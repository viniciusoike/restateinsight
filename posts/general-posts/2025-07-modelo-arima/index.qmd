...
tab_models <- data.frame(
  Modelo = c("ARIMA(0,1,0)", "ARIMA(1,1,1)", "ARIMA(1,1,0)", "ARIMA(0,1,1)"),
  AIC = c(m1$aic, m2$aic, m3$aic, m4$aic),
  AICc = c(m1$aicc, m2$aicc, m3$aicc, m4$aicc),
  BIC = c(m1$bic, m2$bic, m3$bic, m4$bic),
  sigma2 = c(m1$sigma2, m2$sigma2, m3$sigma2, m4$sigma2)
)
```
...
5. **Previsão**: O modelo final foi usado para gerar previsões fora da amostra, com intervalos de confiança apropriados que refletem a incerteza crescente ao longo do horizonte de previsão.
...
- Busca-se um modelo parcimonioso e interpretável

**Limitações importantes:**

- ARIMA assume linearidade nas relações temporais
