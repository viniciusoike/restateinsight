...
tbl_labor <- labor |> 
  ...
  select(
    code_state = unidade_da_federacao_codigo,
    status = condicao_em_relacao_a_forca_de_trabalho_e_condicao_de_ocupacao,
    value = valor,
    quarter = trimestre_codigo) |> 
  mutate(
    date = zoo::as.yearqtr(quarter, format = "%Y%q"),
    date = zoo::as.Date(date)
  ) |> 
  pivot_wider(
    id_cols = c("code_state", "date"),
    names_from = "status",
    values_from = "value"
  )
...
tbl_labor <- tbl_labor |> 
  janitor::clean_names() |> 
  mutate(
    part_rate = forca_de_trabalho / total * 100,
    employment_population = forca_de_trabalho_ocupada / total * 100,
    unemp = forca_de_trabalho_desocupada / forca_de_trabalho,
    unemp_adjusted = forca_de_trabalho_desocupada / (total * index_base$base) * 100
    ) |> 
  pivot_longer(
    cols = -c(code_state, date),
    names_to = "series"
  )
```

```{r}
labor_br <- get_sidra(4092, variable = 1641, period = "201801-202302")

tbl_labor_br <- labor_br |> 
  janitor::clean_names() |> 
  select(
    status = condicao_em_relacao_a_forca_de_trabalho_e_condicao_de_ocupacao,
    value = valor,
  ...
  pivot_wider(id_cols = "date", names_from = "status", values_from = "value") |>
  janitor::clean_names()

tbl_labor_br <- tbl_labor_br |> 
  mutate(
    part_rate = forca_de_trabalho / total * 100,
    employment_population = forca_de_trabalho_ocupada / total * 100,
    unemp = forca_de_trabalho_desocupada / forca_de_trabalho
...
br_labor_rate <- tbl_labor_br |> 
  filter(date >= as.Date("2019-01-01"), date <= as.Date("2019-12-01")) |> 
  summarise(share = mean(part_rate)) |> 
  pull(share)

tbl_labor_br <- tbl_labor_br |> 
  mutate(
    unemp_adjust = forca_de_trabalho_desocupada / (total * br_labor_rate) * 100
    )
```
