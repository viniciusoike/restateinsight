...
Lets compare how Uruguay fared against other LATAM countries. For this comparison I will take: Argentina, Brazil, Paraguay and Chile. I have no idea if is the most ideal comparison group but these are neighbour countries. These countries are also likely to not have missing observations.

I also add the most recent HDI estimate from the UN. Here I simply scrape the wikipedia page for "List of countries by Human Development Index"
...
hdi_clean <- dat |> 
  select(
    country = nation_nation,
    hdi = hdi_2021_data_2022_report_2,
    hdi_growth = hdi_average_annual_growth_2010_2021_20
  ) |> 
  mutate(
    hdi = as.numeric(hdi),
...
Uruguay's HDI is barely in the "very high" zone (>=0.8). It's above both Brazil and Paraguay, but lags behined Argentina and Chile. In terms of real GDP per capita, Uruguay is very close to Argentina and Chile.
