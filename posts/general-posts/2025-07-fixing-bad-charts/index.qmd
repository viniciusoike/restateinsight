...
description: ""
categories: ["data-visualization", "ggplot2", "data-science", "tutorial-R"]
format:
  html:
...
#| code-fold: true
xconv <- c(
  "Melhor" = "Better", "Igual" = "Same", "Pior" = "Worse",
...
O primeiro gráfico mostra a avaliação dos brasileiros a respeito da condição da economia nos últimos doze meses. Os entrevistados tinham três respostas possíveis: a economia está pior, igual ou melhor do que estava há doze meses atrás.
...
O próximo gráfico mostra

O gráfico abaixo é bastante simples, mas repete os mesmos erros do anterior: cores inconsistentes, linhas de grade em excesso e falta de rótulos de dados.

![](/static/images/bad_charts/01satisfacao_com_a_vida.png){fig-align="center" width="65%"}

De maneira geral, o gráfico tem os seguintes problemas:
...
O gráfico final é bastante simples. Essencialmente, eu reduzi o ruído visual, diminuindo o uso de cores e de linhas de grade. As colunas horizontais facilitam a leitura dos dados.
...
-   **Ausência de rótulos de dados** - Não há percentuais exatos nas barras, impossibilitando leitura precisa dos valores

-   **Legenda** - A legenda está na parte inferior e poderia estar melhor integrada.

-   **Cores contraditórias** - Verde para "Sim" (apoio ao impeachment) pode confundir, já que verde geralmente indica algo positivo. Vale notar que as cores usadas são as cores padrão do {ggplot2} o que pode indicar um "gráfico amador" para quem tem maior familiaridade com R.

-   **Título confuso** - Não comunica insight; parece uma frase inacabada.
...
Aproveitando o fato de que o gráfico aborda um tema político, eu explorei as cores e o posicionamento dos dados, com intuito de tornar a leitura dos dados mais imediata.
...
O título agora remete imediatamente ao principal motivo que justificou o impeachment da presidente à época, i.e. as pedaladas fiscais[^3].
...
<https://education.economist.com/insights/interviews/tips-for-visualising-data-like-the-economist>
